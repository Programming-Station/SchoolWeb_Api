...
namespace School.Infrastructure.Seeds
{
    public static class DefaultLocationData
    {
        public static async Task SeedAsync(SchoolDbContext context)
        {
            ...
            {
                var mhId = context.States.FirstOrDefault(s => s.StateCode == "MH")?.Id ?? 1;
                var dlId = context.States.FirstOrDefault(s => s.StateCode == "DL")?.Id ?? 1;
                var upId = context.States.FirstOrDefault(s => s.StateCode == "UP")?.Id ?? 1;
                var kaId = context.States.FirstOrDefault(s => s.StateCode == "KA")?.Id ?? 1;
                var tnId = context.States.FirstOrDefault(s => s.StateCode == "TN")?.Id ?? 1;
                var wbId = context.States.FirstOrDefault(s => s.StateCode == "WB")?.Id ?? 1;
                var gjId = context.States.FirstOrDefault(s => s.StateCode == "GJ")?.Id ?? 1;
                var tgId = context.States.FirstOrDefault(s => s.StateCode == "TG")?.Id ?? 1;
                var rjId = context.States.FirstOrDefault(s => s.StateCode == "RJ")?.Id ?? 1;
